...
extern
fun parse_d2ecl_node (parsingenv, jsv: jsonval): d2ecl_node
...
val node = parse_d2ecl_node (p2env, jsv)
//
in
...
parse_d2eclist
  (p2env, jsv0) = (
  parse_list<d2ecl> (jsv0, lam x =<cloref1> (parse_d2ecl (p2env, x)))
) (* end of [parse_d2eclist] *)
//
(* ****** ****** *)

extern
fun parse_i2mpdec (parsingenv, jsonval): i2mpdec

extern
fun parse_f2undec (parsingenv, jsonval): f2undec

extern
fun parse_v2aldec (parsingenv, jsonval): v2aldec

extern
fun parse_v2ardec (parsingenv, jsonval): v2ardec
(* ****** ****** *)

extern
fun parse_D2Cimpdec (parsingenv, jsonval): d2ecl_node

extern
fun parse_D2Cfundecs (parsingenv, jsonval): d2ecl_node
extern
fun parse_D2Cvaldecs (parsingenv, jsonval): d2ecl_node

extern
fun parse_D2Cvardecs (parsingenv, jsonval): d2ecl_node
(* ****** ****** *)

extern
fun parse_D2Clocal (parsingenv, jsonval): d2ecl_node

extern
fun parse_D2Cextcode (parsingenv, jsonval): d2ecl_node

extern
fun parse_D2Cdcstdecs (parsingenv, jsonval): d2ecl_node
...
case+ name of
//
| "D2Cimpdec" => parse_D2Cimpdec (p2env, jsv2)
//
| "D2Cfundecs" => parse_D2Cfundecs (p2env, jsv2)
| "D2Cvaldecs" => parse_D2Cvaldecs (p2env, jsv2)
| "D2Cvardecs" => parse_D2Cvardecs (p2env, jsv2)
//
| "D2Clocal" => parse_D2Clocal (p2env, jsv2)
//
| "D2Cextcode" => parse_D2Cextcode (p2env, jsv2)
| "D2Cdcstdecs" => parse_D2Cdcstdecs (p2env, jsv2)
...
val d2c = parse_d2cst (p2env.parsingenv_d2cstmap, d2c)
val def = parse_d2exp (p2env, def)
//
in
...
fun parse_v2ardec_init (parsingenv, jsv: jsonval): d2expopt = let
//
...
in
  if len = 0 then None
    ...
    val i0exp = parse_d2exp (parsingenv, jsvs[0])
  in
    Some i0exp
  end
end
...
val init  = parse_v2ardec_init (p2env, init)
//
in
...
val imp = parse_i2mpdec (p2env, jsvs[1])
//
in
  D2Cimpdec (knd, imp)
...
implement parse_D2Cdcstdecs (p2env, jsv0) = let
//
val-JSONarray(jsvs) = jsv0
...
val d2cst = parse_d2cst (p2env.parsingenv_d2cstmap, jsvs1[0])
//
in
...
val f2ds = parse_list<f2undec> (jsvs[2], lam x => parse_f2undec (p2env, x))
//
in
  D2Cfundecs (knd, f2ds)
...
val v2ds = parse_list<v2aldec> (jsvs[1], lam x => parse_v2aldec (p2env, x))
//
in
  D2Cvaldecs (knd, v2ds)
...
val v2ds = parse_list<v2ardec> (jsvs[0], lam x => parse_v2ardec (p2env, x))
//
in
  D2Cvardecs (v2ds)
end // end of [parse_D2Cvaldecs]
...
val-JSONarray(jsvs) = jsv0
val () = assertloc (length(jsvs) >= 2)
val head = parse_d2eclist (p2env, jsvs[0])
val body = parse_d2eclist (p2env, jsvs[1])
//
in
  D2Clocal (head, body)
