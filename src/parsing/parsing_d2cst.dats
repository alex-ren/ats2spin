...
implement
parse_d2cst0
  (s2env, d2cstmap, jsv0) = (parse_d2cst (0, s2env, d2cstmap, jsv0)).0

implement
parse_d2cst
  (max, s2env, d2cstmap, jsv0) = let
//
val-~Some_vt(jsv2) =
...
| list_cons
    (jsv, jsvs) => let
    val '(d2c, max) = parse_d2cst(max, s2env, d2cstmap, jsv) 
  in 
    loop (max, s2env, d2cstmap, jsvs)
...
val max = loop (max, s2env, d2cstmap, jsvs)
in
  '(d2cstmap, max)
